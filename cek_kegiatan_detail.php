<?php
	...
	$idpelanggan=$_POST["detailidpengguna"];
	$idpembayaran=$_POST["pilihandetail"];
...
<html>
	...
	<body>
		...
		<div class="jumbotron bg-primary text-white mb-4 mt-2">
			<div class="container">
				<h1><b>WisataSyS | Cek Bukti Pelaksanaan Kegiatan</b></h1>
				<p class="lead"><b>Daftar Bukti Pelaksanaan Kegiatan Yang Belum Diverifikasi.</b></p>
		...
		<div class="jumbotron bg-light">
			<div class="container">
				...
				echo "<div id='tampilkonfirmasipilihanpaket' class='mb-2'>
						<h3 class='text-center'>Konfirmasi Bukti Pelaksanaan Kegiatan</h3>
						</div>";
				?>
				<div class="row">
				<?php
					...
					$result4	= mysqli_query($conn,"SELECT tourguide.id_tourguide, tourguide.`nama`,paketwisata.tgl_wisata, 
								pembayaran.status_kegiatan, pembayaran.status_pembayaran, pembayaran.acc_tourguide, pembayaran.tipe_pembayaran , travel.id_travel, travel.nama_travel  FROM fasilitas
								INNER JOIN paketwisata ON fasilitas.`id_fasilitas`=paketwisata.`id_fasilitas`
								INNER JOIN pemesanan ON pemesanan.`no_paketwisata`=paketwisata.`no_paketwisata`
								INNER JOIN pembayaran ON pembayaran.`id_pemesanan`=pemesanan.`id_pemesanan`
					...
					$result5	= mysqli_query($conn,"SELECT nama_wisata.nama AS nama_wisata, pelanggan.`id_pelanggan`, pelanggan.`id_pelanggan`, pelanggan.`nama` AS nama_pelanggan, pelanggan.`telpon`,
								...
								pembayaran.status_kegiatan, pembayaran.acc_tourguide, pembayaran.tipe_pembayaran, travel.nama_travel, pembayaran.status_pembayaran, pembayaran.bukti_bayar, pembayaran.bukti_kegiatan
								FROM fasilitas 
								INNER JOIN paketwisata ON fasilitas.`id_fasilitas`=paketwisata.`id_fasilitas`
								INNER JOIN pemesanan ON pemesanan.`no_paketwisata`=paketwisata.`no_paketwisata`
								INNER JOIN pembayaran ON pembayaran.`id_pemesanan`=pemesanan.`id_pemesanan`
					...
					while($row = mysqli_fetch_array($result)){
						...
						while($row4 = mysqli_fetch_array($result4)){
							...
							echo "
															<span>Afiliasi Travel : <b>".$row4['nama_travel']."(ID : ".$row4['id_travel'].")</b><span><br>
						...
						while($row5 = mysqli_fetch_array($result5)){
							echo "							<br>
															<form method='POST' action='suksesaccbukti.php'>
															<span>Pelanggan : <b>".$row5['nama_pelanggan']."(ID : ".$row5['id_pelanggan'].")</b><span><br>
															<span>No Telpon : <b>".$row5['telpon']."</b><span><br><br>
															<lable><b>Bukti Kegiatan</b></lable>
															<div class='card pl-2 pr-2 pt-2 pb-2 mb-2'>
																<img src='".$row5['bukti_kegiatan']."' width='100%'>
															</div>
															<input type='hidden' name='accpembayaran' value='".$row5['id_pembayaran']."'>
															<select id='pilacc' name='statusacc' class='form-control'>
																<option value='Terlaksana' class='inputform1' selected='true'>Terlaksana</option>
															</select>
															<br>
															<input type='submit' id='acc' value='Validasi Bukti Kegiatan'>
															</form>
													";
						}
						echo "						</div>
