<?php
	...
	@$fasilitasawal		=@$_POST["fasilitasawal"];
	@$deskripsiawal		=@$_POST["deskripsiawal"];
	@$hargaawal			=@$_POST["hargaawal"];
	@$perlutgawal		=@$_POST["perlutgawal"];
	@$paxawal			=@$_POST["paxawal"];

	@$fasilitastambahan	=@$_POST["fasilitastambahan"];
	@$deskripsitambahan	=@$_POST["deskripsitambahan"];
	@$hargatambahan		=@$_POST["hargatambahan"];
	@$perlutgtambahan	=@$_POST["perlutgtambahan"];
	@$paxtambahan		=@$_POST["paxtambahan"];
	...
	$nofasilbaru1 =$nofasillama+1;
	
	@$fawal 		= sizeof(@$fasilitasawal);
	@$ftambahan 	= sizeof(@$fasilitastambahan);
	
	if($fawal==0){
		
	}else{
		for($i=0;$i<$fawal;$i++) {
			$fasilitasawal1 = $fasilitasawal[$i];
			$deskripsiawal1 = $deskripsiawal[$i];
			$hargaawal1 = $hargaawal[$i];
			$perlutgawal1 = $perlutgawal[$i];
			$paxawal1 = $paxawal[$i];
			$result = mysqli_query($conn,"INSERT INTO fasilitas (id_fasilitas, id_travel, id_nama_wisata, nama_fasilitas, deskripsi, harga, status, tuntunan, perpax) VALUES ($nofasilbaru, $idtravel, $idwisata, '$fasilitasawal1', '$deskripsiawal1', $hargaawal1, 'Standar', '$perlutgawal1', '$paxawal1');");
			$nofasilbaru=$nofasilbaru+1;
		}
	}
	if($ftambahan==0){
		
	}else{
		for($i=0;$i<$ftambahan;$i++) {
			$fasilitastambahan1 = $fasilitastambahan[$i];
			$deskripsitambahan1 = $deskripsitambahan[$i];
			$hargatambahan1 = $hargatambahan[$i];
			$perlutgtambahan1 = $perlutgtambahan[$i];
			$paxtambahan1 = $paxtambahan[$i];
			$result = mysqli_query($conn,"INSERT INTO fasilitas (id_fasilitas, id_travel, id_nama_wisata, nama_fasilitas, deskripsi, harga, status, tuntunan, perpax) VALUES ($nofasilbaru, $idtravel, $idwisata, '$fasilitastambahan1', '$deskripsitambahan1', $hargatambahan1, 'Tambahan', '$perlutgtambahan1', '$paxtambahan1');");
			$nofasilbaru=$nofasilbaru+1;
		}
	}
?>
		...
		<div class="jumbotron bg-light">
			<div class="container">
				...
				echo "<div id='tampilkonfirmasipilihanpaket' class='mb-2'>
						<h3 class='text-center'>Paket Wisata Baru Anda Berhasil Ditambahkan.</h3>
						</div>";
				?>
			</div>
		</div>
