...
<html>
	...
	<body>
		...
		<div class="jumbotron bg-primary text-white mb-4 mt-2">
			<div class="container">
				<h1><b>WisataSyS | Terima Pesanan Yang Masuk</b></h1>
				<p class="lead"><b>Lihat daftar pesanan yang masuk pada travel anda.</b></p>
			...
			<div class="container">
				...
				<?php
					$result		= mysqli_query($conn,"SELECT nama_wisata.nama AS nama_wisata, pelanggan.`id_pelanggan`, pelanggan.`id_pelanggan`, pelanggan.`nama` AS nama_pelanggan, pelanggan.`telpon`,
								...
								INNER JOIN tourguide ON tourguide.`id_tourguide`=paketwisata.`id_tourguide`
								...
								WHERE paketwisata.id_travel=
								(SELECT travel.id_travel FROM travel INNER JOIN tourguide ON tourguide.id_travel=travel.id_travel WHERE id_tourguide=$idpengguna) 
								AND pembayaran.acc_tourguide='Pending' AND pembayaran.status_pembayaran='Success' AND paketwisata.id_tourguide=$idpengguna
								GROUP BY paketwisata.no_paketwisata;");
					while($row = mysqli_fetch_array($result)){
						echo "
								<div class='col-md-12 mb-2 mt-2'>
									<div class='card pl-2 pr-2 pt-2'>
										<form id='pilihan".$row['id_pembayaran']."' method='POST' action='terimapesan_detail.php'>
					...
					$result1	= mysqli_query($conn1,"SELECT nama_wisata.nama AS nama_wisata, pelanggan.`id_pelanggan`, pelanggan.`id_pelanggan`, pelanggan.`nama` AS nama_pelanggan, pelanggan.`telpon`,
								pembayaran.id_pembayaran, tourguide.id_tourguide, tourguide.`nama`,paketwisata.tgl_wisata,  
								pembayaran.status_kegiatan, pembayaran.acc_tourguide, pembayaran.tipe_pembayaran, travel.nama_travel, pembayaran.status_pembayaran
								FROM fasilitas 
								INNER JOIN paketwisata ON fasilitas.`id_fasilitas`=paketwisata.`id_fasilitas`
								...
								WHERE paketwisata.id_travel=
								(SELECT travel.id_travel FROM travel INNER JOIN tourguide ON tourguide.id_travel=travel.id_travel WHERE id_tourguide=$idpengguna) 
								AND pembayaran.acc_tourguide='Pending' AND pembayaran.status_pembayaran='Success' AND paketwisata.id_tourguide IS NULL
								GROUP BY paketwisata.no_paketwisata;");
					while($row1 = mysqli_fetch_array($result1)){
						echo "
								<div class='col-md-12 mb-2 mt-2'>
									<div class='card pl-2 pr-2 pt-2'>
										<form id='pilihan".$row1['id_pembayaran']."' method='POST' action='terimapesan_detail.php'>
										<h5 class='text-primary'><b>ID Pembayaran ".$row1['id_pembayaran']." (".$row1['tgl_wisata'].")</b></h5>
										<span>Paket Wisata: <b>".$row1['nama_wisata']."</b></span></br>
										<span>Nama Pemesan: <b>".$row1['nama_pelanggan']."</b></span></br>
										<span>Pembayaran Melalui: <b>".$row1['tipe_pembayaran']."</b></span></br>
										<span>Status Pembayaran: <b>".$row1['status_pembayaran']."</b></span></br>
										<span>Status Penerimaan Oleh Tourguide: <b>".$row1['acc_tourguide']."</b></span></br>
										<span>Status Kegiatan : <b>".$row1['status_kegiatan']."</b></span></br>
										<input type='hidden' name='detailidpengguna' value='".$row1['id_pelanggan']."'>
										<input type='hidden' name='pilihandetail' value='".$row1['id_pembayaran']."'>
										<input class='cek' type='submit' value='Cek Detail'>
										</form>
									</div>
								</div>";
